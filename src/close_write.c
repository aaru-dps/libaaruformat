...
/**
 * @file close_write.c
 * @brief Writer finalization for libaaruformat.
 *
 * Contains the write-path serialization helpers and the ::aaruf_finalize_write()
 * orchestrator that flushes all pending in-memory structures (deduplication tables,
 * checksums, track metadata, sector auxiliary data, and the global index) to the
 * on-disk Aaru image when closing a context opened for writing.
 *
 * The reader-side cleanup and the public ::aaruf_close() entry point live in close.c.
 *
 * @see close.c
 */
...
/**
 * @brief Finalize a write-mode image: flush all pending blocks, DDTs, and the index.
 *
 * Called via the context's finalize_write function pointer from ::aaruf_close().
 * Performs the ordered write sequence: header rewrite, current block flush, DDT
 * serialization, checksums, track/optical/tape metadata, media tags, and finally
 * the global index. Also frees writer-only state (deduplication hash map).
 *
 * @param ctx Pointer to an initialized aaruformatContext in write mode.
 * @return AARUF_STATUS_OK on success, or a negative error code on failure.
 * @internal
 */
